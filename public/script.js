...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sendMessage() {
        ...
        const girlfriendReply = getGirlfriendResponse(message);
        
        setTimeout(() => {
            elements.typingIndicator.classList.remove('active');
            addMessage(girlfriendReply, 'Chisa');
            if (state.voiceEnabled) speakText(girlfriendReply);
        }, 600 + Math.random() * 400);
        ...
        } catch (e) { /* Silent fallback - already showing girlfriend reply */ }
    }
